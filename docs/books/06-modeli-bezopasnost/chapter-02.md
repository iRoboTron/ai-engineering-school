...
> **Лабораторная этого урока:** смотрим, как подмена инструкций попадает в запрос, и проверяем защиты.
> - без интернета: [01_podmena.py](labs/tema6-bezopasnost/01_podmena.py) — запуск `python3 01_podmena.py`, разобрана в разделе [«Практика: смотрим, как это выглядит»](#практика-смотрим-как-это-выглядит)
> - на настоящей модели: [открыть в Google Colab](https://colab.research.google.com/github/iRoboTron/ai-engineering-school/blob/main/docs/books/labs/tema6-bezopasnost/01_podmena_instrukciy.ipynb) · [скачать .ipynb](labs/tema6-bezopasnost/01_podmena_instrukciy.ipynb)
...
Дальше — тема о том, как собрать свой проект и честно о нём рассказать.
