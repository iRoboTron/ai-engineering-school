...
4. Последний блок — **слабое место RAG**: вопрос другими словами («разрешение ходить на тренировки») поиск по словам не находит, и бот честно говорит «не знаю». Заглушка с полным документом отвечает — но это упрощение. На живой модели в ноутбуке вышло иначе: с полным документом модель **тоже** ответила «нет в документе». Другие слова — трудность не только поиска, но и самой модели. Помогают поиск по смыслу (тема 3) и эталонный набор с вопросами, заданными разными словами (тема 5).
...
В ноутбуке ты спрячешь факт в длинный документ и спросишь настоящую модель, где он: в начале, в середине и в конце. Потом сравнишь с RAG по `prompt_tokens` и цене. При подготовке лаборатории на документе в 12 тысяч токенов — больше школьный сервер не пропускает — модель нашла факт во всех трёх местах: середина не потерялась. Зато разница в цене огромная: около 11 900 токенов на вопрос против 96 у RAG.
