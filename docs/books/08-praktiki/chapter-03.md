...
> **Лабораторная этого урока:** ревизия двух ботов и починка бота по одной правке с замером.
> - без интернета: [01_revizor.py](labs/tema8-praktiki/01_revizor.py) — запуск `python3 01_revizor.py`, разобрана в разделе [«Практика: ревизия двух ботов»](#практика-ревизия-двух-ботов)
> - на настоящей модели: [открыть в Google Colab](https://colab.research.google.com/github/iRoboTron/ai-engineering-school/blob/main/docs/books/labs/tema8-praktiki/01_ispravlyaem_bota.ipynb) · [скачать .ipynb](labs/tema8-praktiki/01_ispravlyaem_bota.ipynb)
...
А если хочется дальше — есть **продолжение**: [тема 9](../09-istoriya/book.md) и следующие. Там про то, что начинается, когда твоим проектом пользуются по-настоящему: как не переплачивать за историю диалога, как ускорить ответ и что делать, когда модель подводит.
