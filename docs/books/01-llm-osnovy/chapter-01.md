...
> **Лабораторная этого урока:** режем текст на токены и считаем цену запроса и размер окна.
> - без интернета: [01_tokeny.py](labs/tema1-llm-osnovy/01_tokeny.py) — запуск `python3 01_tokeny.py`, разобрана в разделе [«Практика: считаем токены руками»](#практика-считаем-токены-руками)
> - на настоящей модели: [открыть в Google Colab](https://colab.research.google.com/github/iRoboTron/ai-engineering-school/blob/main/docs/books/labs/tema1-llm-osnovy/01_tokeny.ipynb) · [скачать .ipynb](labs/tema1-llm-osnovy/01_tokeny.ipynb)
...
| `программирование` | `п` + `р` + `ограм` + `м` + `иров` + `ание` | 6 |

Это не выдуманный пример: так режет настоящий токенизатор, и в лаборатории ты получишь ровно эти числа сам.
...
Модели учили в основном на английских текстах, поэтому частые английские слова запомнились целиком, а русские приходится собирать из мелких обрывков. Посмотри ещё раз на таблицу выше: `school` — это один токен, а «школа» — четыре. Целая фраза на русском обычно даёт **в 2–4 раза больше токенов**, чем та же мысль по-английски.

Заметь и другое: слово «кот» — это тоже **два** токена, а не один, хотя короче некуда. Токены нарезаны не по словам и не по слогам, а по тому, что часто встречалось в обучающих текстах.
...
### А теперь — настоящий токенизатор

Мини-версия выше повторяет главную идею, но у настоящего токенизатора словарь примерно на 100 000 частых кусочков и готовые правила. Он называется `tiktoken` — это библиотека от OpenAI, которой пользуются настоящие модели. Числа у него настоящие, а не примерные.

Лаборатория (ноутбук): [скачать .ipynb](labs/tema1-llm-osnovy/01_tokeny.ipynb) · [открыть в Google Colab](https://colab.research.google.com/github/iRoboTron/ai-engineering-school/blob/main/docs/books/labs/tema1-llm-osnovy/01_tokeny.ipynb)

Открывать в Colab удобнее всего: устанавливать ничего не нужно, всё работает в браузере. Ты увидишь, на какие кусочки настоящий токенизатор режет фразу «Привет, как дела?», во сколько раз русский текст «дороже» английского и сколько стоит запрос к модели. Токенизатор работает без интернета и без ключей — прямо в ноутбуке.
