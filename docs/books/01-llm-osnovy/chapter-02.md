...
> **Лабораторная этого урока:** генерируем текст с разной температурой и ловим галлюцинации.
> - без интернета: [02_temperatura.py](labs/tema1-llm-osnovy/02_temperatura.py) — запуск `python3 02_temperatura.py`, разобрана в разделе [«Практика: генератор текста с температурой»](#практика-генератор-текста-с-температурой)
> - на настоящей модели: [открыть в Google Colab](https://colab.research.google.com/github/iRoboTron/ai-engineering-school/blob/main/docs/books/labs/tema1-llm-osnovy/02_generaciya.ipynb) · [скачать .ipynb](labs/tema1-llm-osnovy/02_generaciya.ipynb)
...
flowchart TD
    ...
    D --> E["Работает: дать модели источник"]
    D --> G["Работает: проверить ответ отдельно"]
    D --> H["Работает: спросить дважды и сравнить"]
    D --> F["Ненадёжно: просто попросить не выдумывать"]
    style A fill:#2d2d2d,color:#fff
    style B fill:#1a5276,color:#fff
    style C fill:#7d6608,color:#fff
...
Присмотрись к схеме: три способа работают, а один — нет, и он как раз первым приходит в голову.

**Почему просьба не работает.** Кажется очевидным: скажи модели «не знаешь — так и напиши». Но вспомни определение из начала урока: модель **не выполняет указания**, она продолжает текст. Просьба немного меняет то, какое продолжение выглядит правдоподобным, — и только. В лаборатории к этому уроку мы проверим это тремя разными способами, вплоть до прямого вопроса «существует ли такая книга», и модель всё равно ответит «да» про выдуманную.

Эта мысль вернётся ещё дважды: в теме 4 (границы агенту ставят кодом, а не просьбой) и в теме 6 (от обмана защищает устройство системы, а не вежливая инструкция). Запомни её сразу:

> **Просьба к модели — не защита.** Она может помочь, но полагаться на неё нельзя.

А работает вот что:

- **дать модели нужные документы прямо в запрос** — тогда ей не нужно ничего вспоминать (**тема 2**);
- **проверять ответы измеримо**, а не на глаз (**тема 5**);
- **спросить дважды и сравнить** — самый дешёвый приём, доступен прямо сейчас. Про настоящую книгу модель оба раза скажет примерно одно и то же. Про выдуманную сочинит два разных ответа: в одном герой Лёшка и коллективизация, в другом семья Сомониных и северная деревня. Разошлось — значит, сочиняет.
...
- Уговоры («не знаешь — так и скажи») ненадёжны: **просьба к модели — не защита**. Работают источник в запросе (тема 2), измеримая проверка (тема 5) и приём «спроси дважды и сравни».
