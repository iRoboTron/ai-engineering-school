...
Курс кончился, но это, пожалуй, худший момент останавливаться. Три направления, каждое — небольшой шаг от того, что уже есть:

1. **Подключить настоящую модель.** Всюду в курсе мы заменяли модель заглушкой, чтобы был виден механизм. Замени заглушку на настоящий вызов — и весь остальной код останется прежним. Это, кстати, показывает, что ты понял главное: модель — лишь одна деталь системы.
2. **Сделать поиск по смыслу настоящим.** В теме 3 эмбеддинги были проставлены руками. Существуют готовые модели, которые считают их сами.
3. **Расширить эталонный набор.** Скучно звучит, а пользы больше всего: чем больше вопросов, тем честнее число и тем заметнее любые изменения.
...
Оглянись: семь тем назад «ИИ» был чем-то непонятным из новостей. Теперь ты знаешь, что там внутри — токены и вероятности; знаешь, как заставить ИИ отвечать по документам; как искать по смыслу; что агент — это цикл с границами; как измерить качество числом; почему ИИ можно обмануть и что с этим делают. И у тебя есть свой работающий проект.

Этого достаточно, чтобы дальше разбираться самому — а больше от курса и не требуется.
