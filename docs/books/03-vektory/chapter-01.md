...
Ученик сделал поиск по смыслу и удивился: на вопрос «кабинет 204» система выдала все кружки подряд, а нужную строку — не первой. Почему поиск по смыслу здесь сработал плохо?

<details>
<summary>Ответ</summary>

«204» — это точное значение, а не смысл. Для модели эмбеддингов число 204 почти ничем не отличается от 112: оба «про номер кабинета», смысл у них одинаковый. Поэтому все строки про кабинеты оказываются одинаково близкими.

Вот ровно тот случай, где **поиск по словам** (из темы 2) справляется лучше: он найдёт точное совпадение «204». Поэтому в настоящих системах два поиска используют вместе — каждый закрывает слабость другого.
</details>
...
- Зато на точных значениях (номера, коды, названия) он слабее, поэтому оба способа используют вместе.
- Сменил модель эмбеддингов — пересчитывай все документы заново.

## Проверь себя

1. Почему два текста без общих слов могут оказаться «близкими»?
2. Что означает косинусная близость 0,98? А 0,05?
