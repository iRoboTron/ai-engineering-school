...
# %%
def sprosit_pro_kartinku(vopros, url, max_tokens=500):
...
print("Сырой ответ:", tekst[:300], "\n")
verno, vsego = proverit_raspisanie(vytashchit_json(tekst), PRAVDA)
print(f"\nВерно клеток: {verno} из {vsego}")

# %% [markdown]
# **Что посмотреть в выводе:**
#
# 1. **prompt_tokens** — почти всё это картинка: текста в вопросе на два десятка токенов.
# 2. Таблица проверки: крупный чёткий текст модели обычно читают почти без ошибок.
#    Если где-то ❌ — посмотри, что именно: время с одной цифрой не той? Лишний пробел?
...
# %%
print(f"{'размер':>12} {'prompt_tokens':>14} {'$ за 1000 таких':>16} {'верно клеток':>13}")
for storona in (300, 600, 1000):
    ...
    verno, vsego = proverit_raspisanie(vytashchit_json(tekst), PRAVDA, pechatat=False)
    print(f"{razmer_n[0]:>5}×{razmer_n[1]:<6} {usage.prompt_tokens:>14} "
          f"{usage.prompt_tokens * CENA_VHODA / 1e6 * 1000:>16.4f} {verno:>7} из {vsego}")

# %% [markdown]
# **Что посмотреть в выводе:**
#
# 1. **prompt_tokens растёт с размером.** У разных моделей по-разному: у одних почти
#    пропорционально числу точек, у других ступеньками.
# 2. **Точность на маленькой картинке.** На 300 точках буквы становятся мелкими — часть
#    клеток может прочитаться неверно. Где-то между маленькой и большой картинкой есть
#    размер, после которого точность уже не растёт, а цена растёт.
# 3. Именно поэтому совет «уменьшай до 1000–1500 точек» — не про экономию любой ценой,
#    а про поиск этого разумного размера. Проверяют его, как всегда, замером.
...
#    на один-два больше или меньше. Посмотри на последнюю картинку и попробуй сосчитать
...
#    сравним.
# 2. Нашла ли модель факт из середины? Честно: современная модель на документе такой
#    длины часто находит всё. «Потерянная середина» сильнее проявляется на текстах в сотни
...
print(f"\nRAG дешевле в {v_okno / v_rag:.0f} раз")
...
# 2. Итоговая таблица: RAG дешевле в десятки раз — на каждом вопросе. Наша модель дешёвая,
...
# **Что посмотреть в выводе:**
#
# 1. **RAG**, скорее всего, нашёл не те пункты: в вопросе нет слов «пропуск» и «спортзал»,
#    и поиск по словам ухватился за что-то случайное. Модель честно ответила «нет в
#    документе» — и это хорошо, что не выдумала.
# 2. **Всё в окно**, скорее всего, нашла: модель видит весь документ и понимает, что
#    «разрешение на тренировки» — это пропуск в спортзал.
# 3. Отсюда настоящий выбор инженера: не «окно или RAG», а **RAG, который ищет по смыслу**
#    (тема 3), или сочетание поиска по словам и по смыслу. Тогда и дёшево, и понимает
#    другие слова.
